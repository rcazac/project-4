...
let slider_about = new Swiper('.main-slide__items', {
...
function menu_close() {
	let iconMenu = document.querySelector(".nav__menu");
...
let closePo = document.querySelector('.btn-block-calc');
let popClose = document.querySelector('.popup_pop');
let popBody = document.querySelector('body');
closePo.addEventListener('click', function (){

	popClose.classList.remove("_active");
    popBody.classList.remove("_lock");
    popBody.style.padding = 0;
});
...
const offsetts = 500;
const scrollUpss = document.querySelector('.scroll-top ');


const getTops = () => window.pageYOffset || document.documentElement.scrollTop;
...
window.addEventListener('scroll', () => {
  if (getTops() > offsetts) {
    scrollUpss.classList.add ('active');
  } else {
    scrollUpss.classList.remove ('active');
  }
...
scrollUpss.addEventListener('click', () => {
  window.scrollTo({
    top:0,
    behavior:'smooth'
  });
